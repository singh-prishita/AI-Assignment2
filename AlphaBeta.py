...
class AlphaBetaAgent(Player):
    ...
    def _min(self, board: Board, alpha=float('-inf'), beta=float('inf')) -> (float, int):
        """
        Evaluate the board position `board` from the Minimizing player's point of view.

        :param board: The board position to evaluate
        :param alpha: The best value that the maximizing player currently can guarantee at this level or above
        :param beta: The best value that the minimizing player currently can guarantee at this level or above
    ...
    def _max(self, board: Board, alpha=float('-inf'), beta=float('inf'))-> (float, int):
        """
        ...
        if board_hash in self.cache:
         ...
         #
         # Init the max value as well as action. Max value is set to DRAW as this value will pass through in case
         # of a draw
         #
        ...
        if winner == self.side:
            ...
            for index in [i for i, e in enumerate(board.state) if board.state[i] == EMPTY]:
                ...
                res, _ = self._min(b, alpha=alpha, beta=beta)
